...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int i = 0; i < h; i++) {
    b->board[i] = malloc(w * (sizeof(*b->board[i])));  // allocate space
    for (int j = 0; j < w; j++) {
      b->board[i][j] = UNKNOWN;  // fill the UNKOWN
    }
  }
  b->width = w;
  b->height = h;
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    int j = i % 3;
    int k = i / 3;
    /* This func will not check the board[x][y]*/
    if (k == 1 && j == 1) {
      continue;
    }
    /* Compute the nx,ny and check the bounds */
    int nx = x + dir[k];
    int ny = y + dir[j];
