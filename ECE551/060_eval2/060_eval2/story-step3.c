...
int main(int argc, char ** argv) {
  ...
  //The variable to get the line. and get the category array.
  char * cur = NULL;
  size_t sz = 0;
  ...
  //The variable to get the line. To print the result.
  char * cur2 = NULL;
  size_t sz2 = 0;
  ...
  free(cur2);
  //free the record struct. It record the use of words
  for (int i = 0; i < record->n_words; i++) {
    free(record->words[i]);
  }
  free(record->words);
  free(record);
  //free the category array, it record the category kinds.
  ...
  free(catarr->arr);
  free(catarr);
  //close the file1(category)
  if (fclose(f1) != 0) {
    fprintf(stderr, "Can't close the file. \n");
    exit(EXIT_FAILURE);
  }
  //close the file2(story)
  if (fclose(f2) != 0) {
    fprintf(stderr, "Can't close the file. \n");
    exit(EXIT_FAILURE);
  }
