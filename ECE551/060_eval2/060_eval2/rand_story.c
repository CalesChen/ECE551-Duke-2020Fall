...
void alter(
  ...
  while (1) {
    ...
    if (temp1 == NULL) {
      ...
      /* check if the cat is only containing digits, Can use it when necessary*/
      /* char * temp_c = cat;  
      while (*temp_c != '\0') {
        if (!isdigit(*temp_c)) {
          back = 0;
...
void delete_c(char * cat, const char * replace, catarray_t * catarr) {
  ...
  //evaluate the new words pointer.
